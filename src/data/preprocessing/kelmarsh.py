...
edge_attr_latlon = np.array(
    [
        [
            abs(df_static.iloc[i, 9] - df_static.iloc[j, 9]),
...
def compute_eucl_dist(i_1: int, i_2: int):
    dist = np.sqrt(
        abs(df_static.iloc[i_1, 9] - df_static.iloc[i_2, 9]) ** 2
...
edge_attr_eucl = np.array(
    [compute_eucl_dist(i, j) for i in range(6) for j in range(6) if i != j]
)
# np.save('data/processed/kelmarsh/edge_attr.npy', edge_attr_eucl)

# read turbine data
t1 = pd.read_csv(
    "data/raw/kelmarsh/turbine1.csv",
    skiprows=[0, 1, 2, 3, 4, 5, 6, 7, 8],
    index_col=0,
    parse_dates=True,
)
t2 = pd.read_csv(
    "data/raw/kelmarsh/turbine2.csv",
    skiprows=[0, 1, 2, 3, 4, 5, 6, 7, 8],
    index_col=0,
    parse_dates=True,
)
t3 = pd.read_csv(
    "data/raw/kelmarsh/turbine3.csv",
    skiprows=[0, 1, 2, 3, 4, 5, 6, 7, 8],
    index_col=0,
    parse_dates=True,
)
t4 = pd.read_csv(
    "data/raw/kelmarsh/turbine4.csv",
    skiprows=[0, 1, 2, 3, 4, 5, 6, 7, 8],
    index_col=0,
    parse_dates=True,
)
t5 = pd.read_csv(
    "data/raw/kelmarsh/turbine5.csv",
    skiprows=[0, 1, 2, 3, 4, 5, 6, 7, 8],
    index_col=0,
    parse_dates=True,
)
t6 = pd.read_csv(
    "data/raw/kelmarsh/turbine6.csv",
    skiprows=[0, 1, 2, 3, 4, 5, 6, 7, 8],
    index_col=0,
    parse_dates=True,
)
